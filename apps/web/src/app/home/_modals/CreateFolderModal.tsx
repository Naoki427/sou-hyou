...
type Props = {
  ...
  parentPath: string; // 表示用
};

export function CreateFolderModal({ open, onClose, parentId, parentPath }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await createFolder({
        variables: { input: { name: name.trim(), parentId } },
      });
  ...
  return (
    <div style={backdrop} onClick={onClose}>
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        <h3 style={{ marginTop: 0 }}>フォルダ作成</h3>
